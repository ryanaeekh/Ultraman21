...
from utils import safe_float, clean_text as safe_text, load_exercise, save_exercise_df, EXERCISE_COLUMNS
...
def _load_exercise():
    ...
    df = ensure_columns(df, EXERCISE_COLUMNS)
    if not df.empty:
        df["duration"] = df["duration"].apply(safe_float)
        df["km"] = df["km"].apply(safe_float)
    return df


def _invalidate():
    _load_exercise.clear()
...
st.markdown(
    f'<div class="hero-banner"><div class="hero-label">Today\'s Exercise Status</div><div class="hero-value">{hero_text}</div></div>',
...
with m1:
    ...
    st.markdown(metric_card("Today's Run", f"{today_km_val:.2f} km", "Distance logged for today's entry"), unsafe_allow_html=True)
with m2:
    today_dur_val = safe_float(today_row.get("duration", 0)) if today_row is not None else 0
    st.markdown(metric_card("Today's Time", f"{today_dur_val:.0f} min", "Workout / run duration for today"), unsafe_allow_html=True)
with m3:
    st.markdown(metric_card("Total Run To Date", f"{total_km:.2f} km", f"{total_runs} completed session(s) &bull; {total_minutes:.0f} total min"), unsafe_allow_html=True)
...
with left_col:
    ...
    exercise_type = st.selectbox(
        ...
        if default_type in ["Run", "Walk", "Gym", "Cycle", "Stretch", "Recovery", "Other"] else 0
    )
    c1, c2 = st.columns(2)
    with c1:
    ...
    if save_btn:
        ...
        new_row = {
            "date": selected_date_str, "status": status, "type": exercise_type,
            "duration": round(float(duration), 2), "km": round(float(km), 2),
            "pace": pace_value, "notes": notes.strip(),
        }
        ...
        _save_exercise(updated_df)
        st.success("Exercise saved.")
        st.rerun()

    st.markdown('<div class="c-muted" style="font-size:13px;margin-top:8px;">One date = one exercise record.</div>', unsafe_allow_html=True)
...
with right_col:
    ...
    if today_row is None:
        ...
        current_pace = safe_text(today_row.get("pace", "")) or calculate_pace(
            safe_float(today_row.get("duration", 0)), safe_float(today_row.get("km", 0))
        )
...
if exercise_df.empty:
    ...
    display_df = history_df.rename(columns={"date": "Date", "status": "Status", "type": "Type", "duration": "Time (min)", "km": "KM", "pace": "Pace", "notes": "Notes"})
    ...
    if not run_chart_df.empty:
        ...
        st.line_chart(run_chart_df.set_index("date")[["km"]], height=180)
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
