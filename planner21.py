...
with st.sidebar:
    ...
    NAV_PAGES = ["Today", "History", "Insights", "AI Coach", "Settings"]
    page = st.radio(
        "Navigate",
        NAV_PAGES,
...
# =========================================================
    ...
    st.markdown(
        ...
        '<div class="page-sub">Your personal productivity assistant — powered by Claude</div>'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    if not api_key:
        st.warning("⚠️ No API key found. Add your Anthropic API key to the `.env` file in the project root:")
        st.code('ANTHROPIC_API_KEY=sk-ant-...')
        st.info("Get your key at https://console.anthropic.com/")
    else:
        import anthropic

        # ── Helper: load all data for AI context ──
        def _build_data_context():
            ...
            if not pdf.empty:
                ...
                lines.append(f"=== PLANNER (last 7 days) ===\nTotal days logged: {len(pdf)}, Streak: {streak}, Weekly avg: {weekly_avg}/100")
                for _, r in recent.iterrows():
                    lines.append(f"  {r['date']}: score={r['score']}, focus={r['focus_done']}, run={r['run_done']}, income={r['income_done']}, reflection={clean_text(r['reflection'])[:100]}")
            ...
            if os.path.exists(fin_path):
                ...
                if not fdf.empty and "date" in fdf.columns:
                    ...
                    total_spent = round(week_fin["amount"].astype(float).sum(), 2) if not week_fin.empty and "amount" in fdf.columns else 0
            ...
            if os.path.exists(ex_path):
                ...
                if not edf.empty and "date" in edf.columns:
                    ...
                    if not week_ex.empty:
                        done_count = len(week_ex[week_ex["status"].astype(str).str.lower().isin(["done", "completed", "yes"])])
            ...
            if not sdf.empty:
                try:
                    lines.append(f"\n=== TARGETS ===\nDaily income target: ${float(sdf.loc[0, 'daily_income_target']):.0f}, Daily budget: ${float(sdf.loc[0, 'daily_budget']):.0f}, Monthly budget: ${float(sdf.loc[0, 'monthly_budget']):.0f}")
                except Exception:
                    pass
        ...
        # ── Smart Features ──
        st.markdown('<div class="section-label">⚡ Smart Features</div>', unsafe_allow_html=True)
        ...
        with sf1:
            ...
            if st.button("📝 Reflection Prompt", use_container_width=True):
                with st.spinner("Generating reflection prompt..."):
                    client = anthropic.Anthropic(api_key=api_key)
                    today_info = f"Today's score: {today_score}/100. " + (f"Reflection so far: {clean_text(today_row['reflection'])}" if today_row is not None and clean_text(today_row['reflection']) else "No reflection written yet.")
                    resp = client.messages.create(
                        model="claude-sonnet-4-20250514",
                        max_tokens=300,
        ...
        # ── Chat Interface ──
        st.markdown('<div class="section-label">💬 Chat with AI Coach</div>', unsafe_allow_html=True)
